...
port_protocol(container_ports) := pod_protocol if {
	not "protocol" in object.keys(container_ports)
	pod_protocol := "TCP"
}
...
edge_network_relationship[service_name] if {
	...
	service_protocol := port_protocol(service_port)
	...
	# pod protocol
	pod_protocol := port_protocol(pod_ports)
...
edge_network_relationship[service_name] if {
	...
	deployment_protocol := port_protocol(deployment_ports)
	...
	service_name = {
		"destination_name": deployment.name,
		"source_id": source_id,
		"destination_id": destination_id,
		"source_name": service.name,
		"port": service_port,
